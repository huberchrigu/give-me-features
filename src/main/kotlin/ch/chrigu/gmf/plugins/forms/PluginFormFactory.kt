...
class PluginFormFactory {
    suspend fun <PARENT, ID, P : Any> save(title: String, itemDefinition: ItemDefinition<PARENT, ID, P>, pluginData: Map<String, Any?>, uri: String): PluginForm<P> {
        val savedData = itemDefinition.repository.save(itemDefinition.fromMap(pluginData))
    ...
    suspend fun <PARENT : AggregateRoot<ID>, ID, P : Any> create(title: String, itemDefinition: ItemDefinition<PARENT, ID, P>, entity: PARENT, uri: String): PluginForm<P> {
        val pluginData = itemDefinition.repository.findById(entity.id!!) ?: itemDefinition.fromMap(emptyMap())
