...
class TaskModuleTest(private val taskService: TaskService, private val taskRepository: TaskRepository, @MockkBean private val linkedItemProvider: LinkedItemProvider) {
    ...
    fun `should update description`() = runTest {
        ...
        taskService.updateTask(id, task.version!!, Task.TaskUpdate("task", newDescription))
    ...
    fun `should merge tasks`() = runTest {
        val task = taskRepository.save(Task.describeNewTask("test"))
        val askUpdate1 = async { taskService.updateTask(task.id!!, task.version!!, Task.TaskUpdate("new task", Html("new description"))) }
        val askUpdate2 = async { taskService.blockTask(task.id!!, task.version!!) }
    ...
    fun `should block, reopen and close task`() = runTest {
        ...
        val reopened = taskService.reopenTask(task.id!!, blocked.version!!)
        assertThat(reopened.status).isEqualTo(TaskStatus.OPEN)
        assertThat(taskService.closeTask(task.id!!, reopened.version!!).status).isEqualTo(TaskStatus.DONE)
    }
}
